...
- [ ] 第 1 周：完成包结构、接口草案、错误模型和统一命名表。
- [ ] 第 2 周：完成帧/PDU 最小编码解码、`Mock Loopback` 和收发闭环测试。
- [ ] 第 3 周：完成基础扫描流程、SII 最小解析和结构化扫描结果。
- [ ] 第 4 周：完成 ESI 最小解析、ESI/SII 校验、FMMU/SM 首版计算和 `scan/validate` 库层接口草案。

30 天交付标准：
- [ ] 已形成稳定分层骨架。
- [ ] 已形成可测试的 HAL 与帧/PDU 最小闭环。
- [ ] 已形成可输出结构化结果的扫描与配置原型。
- [ ] 已形成可继续拆解 issue 和 commit 的统一任务基线。

## 7. 提交执行方式

- 每完成一个“建议提交拆分”条目，就单独 `git add` 对应文件并提交。
- 提交前先运行本次改动对应的最小验证；文档改动至少检查链接、标题和任务编号是否一致。
- 提交信息优先使用英文祈使句，格式统一为 `type: summary`。
- 只有在一个里程碑的任务全部完成后，才追加一个汇总提交，例如 `docs: mark milestone m1 as complete` 或 `feat: complete milestone m2 minimal loop`。
