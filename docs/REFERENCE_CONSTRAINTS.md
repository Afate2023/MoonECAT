...
| Isochronon identity inspection must not reuse full scan/SII convenience paths | Provider uses one caller absolute deadline and one outstanding PDU; source MAC comes from exact runtime context; only BRD type and SII controller/header words 8..15 are accessed; exact own-TX echo is recorded and ignored in MoonECAT, not Lockwire | `isochronon_provider/identity_operation_wbtest.mbt`: `sim replay fault and native-contract fixtures preserve one deadline and one outstanding request`; `absolute deadline and transaction budget fail closed`; `extra missing reordered model and serial differences fail complete topology`; external `compat/isochronon_provider_consumer` public-boundary test |
