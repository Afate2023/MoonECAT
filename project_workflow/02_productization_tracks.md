...
- [ ] Native：补 SII 完整类别深读计划，按 siitool / SOEM `readeepromAP/FP` 经验继续完善 category 深度解码。当前 `read-sii` 已输出 preamble/standard/header/strings/general/FMMU/FMMU_EX/SM/SyncUnit/DC/PDO/Timeouts/categories/raw-category，不再是仅 header/general 的最小形态；2026-03-16 已修正 strings UTF-8 解码并在真实从站上确认 `group_name/order_name/name` 可读。2026-03-22 先按 ETG.2010 S(R) V1.0.2 补齐了 `Timeouts(70)` 类别解码、AoE mailbox protocol 位、`CurrentOnEBus` 的有符号解释，以及 `read-sii` / `esi-sii` JSON+文本输出与 ESI XML→JSON→SII 视图链路中的 AoE 投影保真（commit: `cdac077`）；随后继续补齐了 `FMMUX(42)`、`SyncUnit(43)`、未知/vendor category 的 `type_code + raw bytes` 保留、Vendor Info、ESC Config Area B，以及 dynamic process-data `SM type 0x05/0x06` 的建模与 CLI 输出，代码提交 `43b36e8` `feat(sii): complete ETG2010 category and header modeling`。验证证据：`moon test` 全量 `406 passed / 0 failed`。
