...
| C-6 | 零拷贝帧池 + ZeroCopyNic HAL 合同 | P2-6 | ❌ | hal/ |
| C-7 | 多平台零拷贝 FFI 后端桩设计 | P2-7 | ❌ | hal/native/ |
| C-8 | ProcessImage 就地映射（消除热路径 Bytes 分配） | P2-8 | ❌ | protocol/, runtime/ |
...
- ❌ 零拷贝帧池 + ZeroCopyNic HAL 合同（C-6）
- ❌ 多平台零拷贝 FFI 后端桩设计（C-7）
- ❌ ProcessImage 就地映射消除热路径分配（C-8）
...
| **MB — Verification Runtime** | 主线 B | B-1 ~ B-6 | ✅ L4 验证加固 |
| **MC — HIL-Ready Boundary** | 主线 C | C-1 ~ C-8 | C-1~C-5 ✅ / C-6~C-8 ❌ |
