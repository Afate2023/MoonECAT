...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 已进入统一产品面尾声收口阶段 | 已新增 runtime 统一配置模型与统一比较路径，CLI/Extism 已支持完整差异项 JSON，CLI 已支持 `validate --eni <path>`，并补上 `Pass/Warn/Block` 分级与最小 ENI `SM/FMMU/PDO + mailbox/DC` 摘要；后续证据聚焦统一拓扑视图与 offline 配置工具模型 |
