...
| 阶段 | 交付物 | 当前状态 | 主要证据 |
|---|---|---|---|
| Phase 0 | Class B 基线能力、库层主链路、基础测试闭环 | 已完成 | [project_workflow/01_baseline_and_roadmap.md](project_workflow/01_baseline_and_roadmap.md) 中 Class B 17/17；`moon test` 通过 |
...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 已验证最小统一产品面 | 已新增 runtime 统一配置模型与统一比较路径，CLI/Extism 已支持完整差异项 JSON，CLI 已改为 `cmd/eni_json` 中转 + `validate --eni-json <path>`，并补上 `Pass/Warn/Block` 分级、单差异 `category/grade` 与最小 ENI `SM/FMMU/PDO + mailbox/DC` 摘要 |
