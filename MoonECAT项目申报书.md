...
本项目依据 **ETG.1500 Master Classes** 标准推进，采用“**Class B 全量基线已完成，Class A 为后续目标能力**”的分阶段策略。第一阶段确保基础主站能力与 Native/CLI 交付闭环，第二阶段围绕配置工具、ENI、完整 CoE 浏览与主站对象字典等 Class A 能力持续收口。
...
| 协议核心 | 服务数据对象（CoE） | 支持 SDO Upload/Download，并逐步补齐 Segmented Transfer、Complete Access、SDO Info 全流程。 |
...
CLI 关键能力：统一参数模型、`free run/dc` 双模式运行。当前已具备 `scan/validate/run/read-sii/state/diagnosis` 入口，后续补齐 ENI 导入、对象字典浏览与更完整的配置校验表面。
...
| 配置工具 / ENI 主线 | 以 online scan 与 ENI import 进入同一配置对象模型为准 | 能输出统一拓扑/配置视图，并给出启动差异报告 |
| 对象字典浏览产品面 | 以现有 SDO Info / Complete Access / Segmented Transfer 能力是否被 CLI 或配置工具直接消费为准 | 能浏览 OD list、OD 描述、OE 描述，并形成稳定输出模型 |
...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 进行中 | 当前已完成 online scan，后续证据将以统一拓扑/配置视图与差异报告为准 |
| Phase 3 | 对象字典浏览产品化：CLI / 配置工具可直接消费的浏览表面 | 进行中 | 库层 SDO Info / Complete Access / Segmented Transfer 已完成，后续证据将以稳定 CLI / JSON 输出为准 |
...
- **配置工具与 ENI 工作流**：在在线扫描之外，补齐 ENI 导入、离线配置生成、启动时网络对比和差异报告，使 MoonECAT 能承载完整工程流，而不只是一套通信库。
- **对象字典浏览产品化**：库层已具备分段传输、Complete Access、SDO Info 与对象字典描述读取能力，下一步把这些能力上浮为 CLI 与配置工具可直接消费的稳定表面。
