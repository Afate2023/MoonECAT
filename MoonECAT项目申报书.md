...
| 配置工具 / ENI 主线 | 以 online scan 与 ENI import 进入同一配置对象模型为准 | 当前已冻结统一配置模型骨架与统一比较路径，并补上最小 ENI XML 投影；后续继续补统一视图和启动差异报告 |
...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 已进入 ENI 接入阶段，继续收口 | 已新增 runtime 统一配置模型与统一比较路径，并补上最小 ENI XML 投影，online scan / 离线期望配置 / ENI import 不再分叉；后续证据聚焦统一拓扑视图、差异分级与更完整 ENI 摘要 |
