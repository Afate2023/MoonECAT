...
| 配置工具 / ENI 主线 | 以 online scan 与 ENI import 进入同一配置对象模型为准 | 当前已冻结统一配置模型骨架与统一比较路径；后续补 ENI import、统一视图和启动差异报告 |
...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 已进入统一模型阶段，继续收口 | 已新增 runtime 统一配置模型与统一比较路径，online scan 和离线期望配置不再分叉；后续证据聚焦 ENI import、统一拓扑视图与差异报告 |
...
- **配置工具与 ENI 工作流**：在在线扫描之外，补齐 ENI 导入、离线配置生成、启动时网络对比和差异报告，使 MoonECAT 能承载完整工程流，而不只是一套通信库。当前已先冻结统一配置模型与统一比较路径，避免后续 ENI 接入时再形成第二套校验语义。
