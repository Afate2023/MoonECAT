...
CLI 关键能力：统一参数模型、`free run/dc` 双模式运行。当前已具备 `scan/validate/run/read-sii/state/diagnosis/od` 入口，其中 `validate --eni <path>` 已可把离线 ENI 文件接入统一比对路径，并输出 `Pass/Warn/Block` 结论、来源信息与完整差异项；ENI 侧也已补到最小 `SM/FMMU/PDO + mailbox/DC` 摘要。
...
| 配置工具 / ENI 主线 | 以 online scan 与 ENI import 进入同一配置对象模型为准 | 当前已接入 `validate --eni <path>`、统一比较路径与 `Pass/Warn/Block` 分级，并补上完整差异项 JSON 和最小 ENI XML + `SM/FMMU/PDO + mailbox/DC` 摘要；后续继续补统一视图与 offline 配置工具模型 |
...
| Phase 2 | 配置工具 / ENI 主线：统一配置对象、差异报告、配置工作流 | 已验证最小统一产品面 | 已新增 runtime 统一配置模型与统一比较路径，CLI/Extism 已支持完整差异项 JSON，CLI 已支持 `validate --eni <path>`，并补上 `Pass/Warn/Block` 分级、单差异 `category/grade` 与最小 ENI `SM/FMMU/PDO + mailbox/DC` 摘要 |
| Phase 3 | 对象字典浏览产品化：CLI / 配置工具可直接消费的浏览表面 | 已验证最小产品面 | Native `od` 命令已可输出 `OD List` / `OD Description` / `OE Description` 的文本与 JSON 结果，并能区分 `transport-failure` / `protocol-failure` / `unsupported`；当前证据已足以支撑配置工具直接消费浏览结果 |
| Phase 4 | 主站对象字典与统一诊断表面 | 已验证最小统一结果对象 | runtime 已新增统一 `DiagnosticSurface` / `DiagnosticIssue` 结果对象，CLI `validate/state/diagnosis/run` 与 Extism `validate/run` 已输出共享 `surface`，并以回放/白盒测试覆盖同一状态结论与错误分级 |
...
- **配置工具与 ENI 工作流**：在在线扫描之外，补齐 ENI 导入、离线配置生成、启动时网络对比和差异报告，使 MoonECAT 能承载完整工程流，而不只是一套通信库。当前已先冻结统一配置模型与统一比较路径，并补上最小 ENI XML 投影、统一差异分级与共享 JSON 结果对象，避免后续继续扩展 ENI 字段时再形成第二套校验语义；下一步聚焦更完整 offline 配置工具模型。
- **对象字典浏览产品化**：库层能力已上浮出最小 Native CLI `od` 浏览表面，可直接浏览 `OD List` / `OD Description` / `OE Description`，失败路径也已形成稳定分类；下一步聚焦配置工具复用与实机 smoke 证据。
- **主站对象字典与诊断汇聚**：最小统一 `DiagnosticSurface` 已落仓，承载 AL Status、WKC、Diagnosis History、运行态遥测与配置摘要，并已由 CLI / Extism 共享；下一步主要剩真实从站 smoke 与更完整主站对象字典视图。
