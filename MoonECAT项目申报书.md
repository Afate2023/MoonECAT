...
CLI 关键能力：统一参数模型、`free run/dc` 双模式运行。当前已具备 `scan/validate/run/read-sii/state/diagnosis/od` 入口，后续补齐 ENI 导入、对象字典错误分类与更完整的配置校验表面。
...
| 对象字典浏览产品面 | 以现有 SDO Info / Complete Access / Segmented Transfer 能力是否被 CLI 或配置工具直接消费为准 | CLI 已能浏览 OD list、OD 描述、OE 描述，并形成稳定输出模型；后续补错误分类与配置工具复用 |
...
| Phase 3 | 对象字典浏览产品化：CLI / 配置工具可直接消费的浏览表面 | 已形成最小 CLI 交付，继续收口 | Native `od` 命令已可输出 `OD List` / `OD Description` / `OE Description` 的文本与 JSON 结果；后续证据聚焦错误分类与配置工具复用 |
...
- **对象字典浏览产品化**：库层能力已上浮出最小 Native CLI `od` 浏览表面，可直接浏览 `OD List` / `OD Description` / `OE Description`；下一步聚焦错误分类、配置工具复用与实机 smoke 证据。
